...
// YOUR DATABASE FILE - use Railway Volume if available, fallback to local
const DB_FILE = process.env.RAILWAY_VOLUME_MOUNT_PATH 
  ? path.join(process.env.RAILWAY_VOLUME_MOUNT_PATH, 'fluxauth-data.json')
  : path.join(__dirname, 'fluxauth-data.json');
...
async function loadDb() {
  try {
    ...
    console.log('Database not found, creating new:', DB_FILE);
...
async function saveDb(data) {
  try {
    ...
    console.log('Database saved:', DB_FILE, 'Users:', data.users.length, 'Keys:', data.keys.length);
  } catch (e) {
...
// Admin: Unban specific key
app.post('/api/admin/keys/:key/unban', async (req, res) => {
  const { key } = req.params;
  const db = await loadDb();
  ...
  db.keys[keyIndex].banReason = null;
  db.keys[keyIndex].unbannedAt = new Date().toISOString();
  db.keys[keyIndex].active = true;
  
  await saveDb(db);
  res.json({ success: true, message: 'Key unbanned', key });
});
...
app.get('/api/admin/keys', async (req, res) => {
  ...
  // Map keys with username
  let keys = db.keys.map(k => {
  ...
  const now = new Date();
  if (filter === 'banned') {
    keys = keys.filter(k => k.banned);
  } else if (filter === 'active') {
    keys = keys.filter(k => k.active && !k.banned && (!k.expiry || new Date(k.expiry) > now));
  } else if (filter === 'expired') {
    keys = keys.filter(k => k.expiry && new Date(k.expiry) < now);
  }
  
  // Calculate stats
  const stats = {
    total: db.keys.length,
    banned: db.keys.filter(k => k.banned).length,
    active: db.keys.filter(k => k.active && !k.banned && (!k.expiry || new Date(k.expiry) > now)).length
  };
