...
app.post('/api/auth/validate', (req, res) => {
  ...
  // IP-based locking (prevents key sharing across networks)
  if (clientIp && clientIp !== 'unknown') {
    if (keyData.lockedIp && keyData.lockedIp !== clientIp) {
      return res.json({ 
        success: false, 
        message: `IP mismatch - key locked to different IP (${keyData.lockedIp})`,
        subscription: 'Expired',
        currentIp: clientIp
      });
    }
    if (!keyData.lockedIp) {
      keyData.lockedIp = clientIp; // Lock to first IP
      console.log(`Key ${actualKey} locked to IP: ${clientIp}`);
    }
  }
  ...
  res.json({
    ...
    owner: keyData.owner,
    name: keyData.name,
    lockedIp: keyData.lockedIp || null,
    hwid: keyData.hwid ? true : false
  });
});
